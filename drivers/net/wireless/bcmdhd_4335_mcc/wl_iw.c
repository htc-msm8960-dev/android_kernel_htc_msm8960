...
static int
wl_iw_set_priv(struct net_device *dev, struct iw_request_info *info, struct iw_point *dwrq, char *ext);
...
wl_iw_attach(struct net_device *dev, void * dhdp)
{
	...
	PROC_START(_iscan_sysioc_thread, iscan, &iscan->tsk_ctl, 0, "_iscan_sysioc_thread");
...
int set_apsta_cfg(struct net_device *dev, struct ap_profile *ap)
{
        ...
        if (ap_cfg_running == FALSE) {
                PROC_START(thr_wait_for_2nd_eth_dev, dev, &ap_eth_ctl, 0, "thr_wait_for_2nd_eth_dev");
        } else {
