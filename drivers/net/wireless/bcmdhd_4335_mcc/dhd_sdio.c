...
static uint get_mac_from_wifi_nvs_ram(char* buf, unsigned int buf_len);
static uint modify_mac_attr(char* buf, unsigned buf_len, char *mac, unsigned int mac_len);
...
dhdsdio_clk_kso_enab(dhd_bus_t *bus, bool on)
{
	...
	do {
#if defined(CUSTOMER_HW_ONE) && defined(MMC_ERROR_WAR)
		if (mmc_tuning_fail) {
...
#ifdef CUSTOMER_HW_ONE
		if ((bus->dhd->busstate == DHD_BUS_DOWN) || bus->dhd->hang_was_sent || bus->dhd->os_stopped) {
	...
	do {
		...
		if (ret == BCME_NODEVICE) {
			...
			if (!dhd_mmc_suspend) {
				...
				DHD_ERROR(("%s: mmc isn't in suspend state, send hang event\n", 
					__FUNCTION__));
				dhd_os_send_hang_message(bus->dhd);
			} else {
	...
	if (ret)
		...
		if (ret) {
			...
			if (bus_txctl_failed_num >= 3) {
				...
				DHD_ERROR(("%s, bus_txctl_failed_num >= 3, Event HANG sent up\n", __FUNCTION__));
...
dhdsdio_read_control(dhd_bus_t *bus, uint8 *hdr, uint len, uint doff)
{
	...
	if (sdret < 0) {
		...
		if ( rx_sdio_err_cnt > MAX_SDIO_ERR_COUNT ) {
			DHD_ERROR(("%s: rx_sdio_err_cnt > %d\n", __FUNCTION__, MAX_SDIO_ERR_COUNT));
			dhd_info_send_hang_message(bus->dhd);
			rx_sdio_err_cnt=0;
		}
#endif
	...
	if (bus->glom) {
		...
		if (errcode < 0) {
			...
			if (rxfail_count > rxglom_fail_count) {
				DHD_ERROR(("%s: rxfail_count > %d\n", __FUNCTION__, rxglom_fail_count));
				dhd_info_send_hang_message(bus->dhd);
				rxfail_count = 0;
			}
#endif
			...
			if (!(pkt = PKTGET(osh, rdlen + DHD_SDALIGN, FALSE))) {
				...
				if (sdret < 0) {
					...
					if ( rx_readahead_err_cnt > MAX_SDIO_ERR_COUNT ) {
						DHD_ERROR(("%s: rx_readahead_err_cnt > %d\n", __FUNCTION__, MAX_SDIO_ERR_COUNT));
						dhd_info_send_hang_message(bus->dhd);
...
#ifdef CUSTOMER_HW_ONE
		if (bus->dhd->hang_was_sent || bus->dhd->os_stopped) {
			resched = 0;
			bus->dpc_sched = resched;
			bus->intstatus = 0;
			goto exit2;
	...
	uint16 func, uint bustype, void *regsva, osl_t * osh, void *sdh)
{
	...
	dhd_readahead = TRUE;
	retrydata = FALSE;
#if defined(PLATFORM_MPS) || defined(CUSTOMER_HW_ONE)
	dhd_doflow = TRUE;
#else
	dhd_doflow = FALSE;
#endif 
...
#if defined(BCM4335_CHIP)
	if (((uint16)bus->sih->chip == BCM4335_CHIP_ID) && (bus->sih->chiprev != 0)) {
