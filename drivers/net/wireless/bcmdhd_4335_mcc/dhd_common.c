...
#ifdef CUSTOMER_HW_ONE
void print_pkt_filter_list(dhd_pub_t *, struct bcmstrbuf *);
void print_btc_mode(dhd_pub_t *, struct bcmstrbuf *);
#endif
static int
dhd_dump(dhd_pub_t *dhdp, char *buf, int buflen)
{
    ...
    print_pkt_filter_list(dhdp, strbuf);
    
    print_btc_mode(dhdp, strbuf);
...
void
print_pkt_filter_list(dhd_pub_t * dhdp, struct bcmstrbuf *strbuf)
{
    ...
    if (!dhdp) {
        bcm_bprintf(strbuf,"print_get pkt_filter_list: dhdp is null");
        return;
    }
    bcm_mkiovar("pkt_filter_list", (char *)&enable, 4, buf, sizeof(buf));
    if ((ret = dhd_wl_ioctl_cmd(dhdp, WLC_GET_VAR, buf, sizeof(buf), FALSE, 0)) < 0) {
        bcm_bprintf(strbuf,"print_pkt_filter_list: ret = %d", ret);
        return;
    }
    else {
        list = (wl_pkt_filter_list_t *) buf;
        filterp = list->filter;
        if (list->num > 0 ) {
            bcm_bprintf(strbuf, "pkt_filter_list 1 ID: ");
            for (i = 0; i < list->num; i++) {
                bcm_bprintf(strbuf, "%d ", dtoh32(filterp->id));
...
void
print_btc_mode(dhd_pub_t * dhdp, struct bcmstrbuf *strbuf)
{
    char buf[16];
    int ret;

    if (!dhdp) {
        bcm_bprintf(strbuf,"print_btc_mode: dhdp is null");
        return;
    }
    bcm_mkiovar("btc_mode", 0, 0, buf, sizeof(buf));
    if ((ret = dhd_wl_ioctl_cmd(dhdp, WLC_GET_VAR, buf, sizeof(buf), FALSE, 0)) < 0) {
        bcm_bprintf(strbuf,"print_btc_mode: ret = %d", ret);
        return;
    }
    else {
        uint32 btc_mode_val = 0;
        memcpy(&btc_mode_val, buf, sizeof(btc_mode_val));
        bcm_bprintf(strbuf, "btc_mode=%d \n", btc_mode_val);
    }
    return;
}
