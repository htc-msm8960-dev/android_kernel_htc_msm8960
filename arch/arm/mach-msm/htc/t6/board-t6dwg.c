...
static void __init t6dwg_reserve(void)
{
	msm_reserve();
}

static void __init t6dwg_early_reserve(void)
{
...
static int64_t t6dwg_get_usbid_adc(void)
{
...
static void t6dwg_config_usb_id_gpios(bool output)
{
	int rc;
...
static void t6dwg_usb_dpdn_switch(int path);
#endif

static struct cable_detect_platform_data cable_detect_pdata = {
	.detect_type		= CABLE_TYPE_PMIC_ADC,
	.usb_id_pin_gpio	= PM_USB1_HS_ID_GPIO,
	.get_adc_cb		= t6dwg_get_usbid_adc,
	.config_usb_id_gpios	= t6dwg_config_usb_id_gpios,
#ifdef CONFIG_FB_MSM_HDMI_MHL
	.mhl_1v2_power		= mhl_sii9234_1v2_power,
	.usb_dpdn_switch	= t6dwg_usb_dpdn_switch,
#endif
};
...
static void t6dwg_cable_detect_register(void)
{
	int rc;
...
void t6dwg_pm8xxx_adc_device_register(void)
{
	pr_info("%s: Register PM8XXX ADC device. rev: %d\n",
		__func__, system_rev);
	t6dwg_cable_detect_register();
}

void t6dwg_add_usb_devices(void)
{
...
static struct max1187x_virtual_key t6dwg_vk_data[] = {
	{
		.index = 0,
		.keycode = KEY_HOME,
		.x_position = 1770,
...
static struct platform_device mdm_t6dwg_device = {
	.name		= "mdm2_modem",
	.id		= -1,
...
static void __init t6dwg_map_io(void)
{
	pr_info("%s: init starts\r\n", __func__);
...
static void __init t6dwg_init_irq(void)
{
...
static struct platform_device msm8064_device_saw_regulator_core0 = {
	...
	.id	= 0,
	.dev	= {
		.platform_data = &t6china_saw_regulator_pdata_8921_s5,
	},
};
...
static struct platform_device msm8064_device_saw_regulator_core1 = {
	...
	.id	= 1,
	.dev	= {
		.platform_data = &t6china_saw_regulator_pdata_8921_s6,
	},
};
...
static struct platform_device msm8064_device_saw_regulator_core2 = {
	...
	.id	= 2,
	.dev	= {
		.platform_data = &t6china_saw_regulator_pdata_8821_s0,
	},
};
...
static struct platform_device msm8064_device_saw_regulator_core3 = {
	...
	.id	= 3,
	.dev	= {
		.platform_data = &t6china_saw_regulator_pdata_8821_s1,
	},
};
...
static void __init t6dwg_init_buses(void)
{
...
static struct platform_device t6china_device_ext_5v_vreg __devinitdata = {
	.name	= GPIO_REGULATOR_DEV_NAME,
	.id	= PM8921_MPP_PM_TO_SYS(7),
	.dev	= {
		.platform_data
			= &t6china_gpio_regulator_pdata[GPIO_VREG_ID_EXT_5V],
	},
};

static struct platform_device t6china_device_ext_mpp8_vreg __devinitdata = {
	.name	= GPIO_REGULATOR_DEV_NAME,
	.id	= PM8921_MPP_PM_TO_SYS(8),
	.dev	= {
		.platform_data
			= &t6china_gpio_regulator_pdata[GPIO_VREG_ID_EXT_MPP8],
	},
};

static struct platform_device t6china_device_ext_ts_sw_vreg __devinitdata = {
	.name	= GPIO_REGULATOR_DEV_NAME,
	.id	= PM8921_GPIO_PM_TO_SYS(23),
	.dev	= {
		.platform_data
			= &t6china_gpio_regulator_pdata[GPIO_VREG_ID_EXT_TS_SW],
	},
};

static struct platform_device t6china_device_rpm_regulator __devinitdata = {
	.name	= "rpm-regulator",
	.id	= -1,
	.dev	= {
		.platform_data = &t6china_rpm_regulator_pdata,
	},
};
...
static void __init t6dwg_receiver_init(void)
{
...
static struct cir_platform_data t6dwg_cir_gsbi3_pdata = {
	.cir_reset = cir_reset,
	.cir_power = cir_power,
};
...
static struct platform_device *common_devices[] __initdata = {
	...
	&msm_device_uart_dm4,
#endif
	&t6china_device_ext_5v_vreg,
	&t6china_device_ext_mpp8_vreg,
	&t6china_device_ext_ts_sw_vreg,
...
#define FPC1080_RESET_GPIO IOEXT_GPIO_3 
#define FPC_IRQ_GPIO FP_INT
#define FPC_PIN_GPIO_SPI_CS2 FP_SPI_CS0
#define FPC_PIN_GPIO_SPI_CLK MCAM_FP_SPI_CLK
#define FPC_PIN_GPIO_SPI_DI  MCAM_FP_SPI_DI
#define FPC_PIN_GPIO_SPI_DO  MCAM_FP_SPI_DO
...
static struct msm_gpiomux_config t6dwg_fpc_common_configs[] = {
...
#if 0
	{
		.gpio      = FPC_PIN_GPIO_SPI_CLK,
		.settings = {
			[GPIOMUX_ACTIVE] = &fpc_settings[1], 
			[GPIOMUX_SUSPENDED] = &fpc_settings[0], 
		},
	},
#endif
	{
		.gpio      = FPC_PIN_GPIO_SPI_CS2,
		.settings = {
			[GPIOMUX_ACTIVE] = &fpc_settings[5], 
			[GPIOMUX_SUSPENDED] = &fpc_settings[0], 
		},
	},
#if 0
	{
		.gpio      = FPC_PIN_GPIO_SPI_DI,
		.settings = {
			[GPIOMUX_ACTIVE] = &fpc_settings[1], 
...
static uint16_t t6dwg_fpc_gpio[] = {
	FPC_IRQ_GPIO,
	FPC_PIN_GPIO_SPI_CS2,
};
...
static int fingerprint_set_pin(int pin_num, int value)
{
        int rc = 0;
		printk("[fp] %s start\n", __func__);
        rc = ioext_gpio_set_value(pin_num, value);
		printk("[fp] %s end\n", __func__);
        return rc;
}
...
static int fingerprint_set_power_control(int power_state)
{
        ...
        if(power_state == 1) 
        {
            ...
            rc = ioext_gpio_set_value(0, 1);
            rc = ioext_gpio_get_value(0);
            if(rc != 1)
            {
            ...
            rc = ioext_gpio_set_value(1, 1);
            rc = ioext_gpio_get_value(1);
            if(rc != 1)
            {
...
static int __init t6dwg_validity_fingerprint_spi_init(void)
{
	int err;
	int i;
	int rc = 0;

	printk("[fp] t6dwg validity FPR init start\n");
	...
	msm_gpiomux_install(t6dwg_fpc_common_configs, ARRAY_SIZE(t6dwg_fpc_common_configs));
	for (i = 0; i < ARRAY_SIZE(t6dwg_fpc_gpio); i++) {
		rc = gpio_request(t6dwg_fpc_gpio[i], "vfsspi");
		if (rc < 0) {
			pr_err("[fp] %s not able to get gpio\n", __func__);
			for (i--; i >= 0; i--)
				gpio_free(t6dwg_fpc_gpio[i]);
			break;
		}
	}
	rc = gpio_direction_input(FPC_IRQ_GPIO);
	printk("[fp] t6dwg validity FPR init end gpio55 = %d\n", rc);
...
static void __init t6dwg_i2c_init(void)
{
...
static struct mpu3050_platform_data mpu3050_data = {
	...
	.power_LPM = t6dwg_mpu3050_sensor_power_LPM,
};
...
static T_MHL_PLATFORM_DATA mhl_sii9234_device_data = {
	...
	.ci2ca = 0,
	.mhl_usb_switch = t6dwg_usb_dpdn_switch,
	.mhl_1v2_power = mhl_sii9234_1v2_power,
...
static struct i2c_registry t6dwg_i2c_devices[] __initdata = {
	{
		I2C_SURF | I2C_FFA,
...
static void __init register_i2c_devices(void)
{
	...
	for (i = 0; i < ARRAY_SIZE(t6dwg_i2c_devices); ++i) {
		if (t6dwg_i2c_devices[i].machs & mach_mask) {

			i2c_register_board_info(t6dwg_i2c_devices[i].bus,
						t6dwg_i2c_devices[i].info,
...
static void __init t6dwg_common_init(void)
{
	int rc = 0;
	...
	platform_device_register(&t6china_device_rpm_regulator);
	if (msm_xo_init())
		pr_err("Failed to initialize XO votes\n");
	msm_clock_init(&apq8064_clock_init_data);
	t6china_init_gpiomux();
	...
	t6dwg_validity_fingerprint_spi_init();
#endif
	pm_qos_add_request(&pm_qos_req_dma, PM_QOS_CPU_DMA_LATENCY, PM_QOS_DEFAULT_VALUE);

}

static void __init t6dwg_allocate_memory_regions(void)
{
	t6_allocate_fb_region();
}

static void __init t6dwg_cdp_init(void)
{
...
MACHINE_START(T6_DWG, "UNKNOWN")
	.fixup = t6dwg_fixup,
	.map_io = t6dwg_map_io,
	.reserve = t6dwg_reserve,
	.init_irq = t6dwg_init_irq,
	.handle_irq = gic_handle_irq,
	.timer = &msm_timer,
	.init_machine = t6dwg_cdp_init,
	.init_early = t6dwg_allocate_memory_regions,
	.init_very_early = t6dwg_early_reserve,
	.restart = msm_restart,
MACHINE_END
